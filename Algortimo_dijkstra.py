...
def parsear_conexiones(texto, nodo_origen, nodos_validos):
    ...
    - NODO no puede ser el mismo nodo_origen (auto-loop)
        ...
        while True:
            try:
                ...
                for destino, peso in conexiones:
                    if not es_dirigido:
                        # Verificar si la arista inversa ya existe (agregada automaticamente)
...
def dijkstra(grafo, origen):
    ...
    Usa implementacion manual sin heap (O(n^2)), sin librerias.
    """
    INF = float('inf')
...
def mostrar_resultado(camino, distancia, origen, destino):
    ...
    print()
    if origen == destino:
        print(f"  Resultado: {origen} | peso: 0")
        return
    ...
    # Formatear peso: mostrar entero si no tiene decimales
    if distancia == int(distancia):
        peso_str = str(int(distancia))
    else:
        peso_str = str(round(distancia, 4))

    print(f"  Resultado: {camino_str} | peso: {peso_str}")
